...
def main():
    # 1. Read A3 events and initialize
    folder = "./Human_Mouse/0_m_ss_score/m_result_1000"
    print(f"Folder path: {folder}")
    a3_df = pd.read_csv("./data/mm10_A3.txt", sep="\t")
    a3_df["index"]    = -1
    a3_df["prob_5ss"] = -1.0
    a3_df["prob_3ss"] = -1.0
    ...
    for idx, grp in df_extremes.groupby("index"):
        if grp["type"].nunique() == 2:
            y_minor = grp.loc[grp["type"] == "minor", "prob_3ss"].values[0]
            y_major = grp.loc[grp["type"] == "major", "prob_3ss"].values[0]
    ...
    ax1.set_xlabel("Alternative 3'SS", fontsize=40)
    ax1.set_xticks([0, 1])
    ...
    ax2.legend(
        ...
        bbox_to_anchor=(-0.33, 1.05),  # 往上移一點（y > 1）
        fontsize=40,
        frameon=False
    )
    ...
    plt.tight_layout()
    plt.savefig("./A3_result/A3_m_k.png", dpi=400)
    plt.show()
    print(f"r = {r:.2f}\n(p = {p:.2g})")
...
#     # folder = "t_result_ss_1000"
#     # print(f"Folder path: {folder}")
#     # a3_df = pd.read_csv("./data/tair10_A3.txt", sep="\t")
...
#     # --- Left: transparent boxplot + colored swarm, swapped order ---
#     sns.boxplot(
#         x="type", y="prob_3ss", data=df_extremes,
#         order=["minor", "major"],
#         showfliers=False,
...
#         data=df_extremes[df_extremes["type"] == "major"],
#         order=["minor", "major"],
#         color="blue", size=6, ax=ax1
#     )
#     sns.swarmplot(
#         x="type", y="prob_3ss",
#         data=df_extremes[df_extremes["type"] == "minor"],
#         order=["minor", "major"],
#         color="orange", size=6, ax=ax1
#     )
#     ax1.set_ylabel("Alternative 3'SS Score", fontsize=20)
#     ax1.set_xlabel("Alternative 3'SS", fontsize=20)
#     ax1.set_xticks([0, 1])
#     ax1.set_xticklabels(
#         ["Minor 3'SS", "Major 3'SS"],
#         fontsize=20
#     )
#     ax1.xaxis.set_tick_params(labelsize=20)
#     ax1.yaxis.set_tick_params(labelsize=20)

#     # --- Right: scatter + Pearson legend ---
#     jitter = 0.5
#     x = df_extremes["PSI"] + np.random.normal(scale=jitter, size=len(df_extremes))
#     y = df_extremes["prob_3ss"]
#     colors = df_extremes["type"].map({"major":"blue","minor":"orange"})
...
#     ax2.set_xlabel("PSI", fontsize=20)
#     ax2.set_ylabel("Alternative 3'SS Score", fontsize=20)

#     legend_text = f"r = {r:.2f} (p = {p:.2g})"
#     custom_legend = [Line2D([0], [0], color='none', label=legend_text)]
#     ax2.legend(
#         handles=custom_legend,
#         loc='upper left',
#         bbox_to_anchor=(-0.23, 1.02),
#         fontsize=20,
#         frameon=False
#     )
...
#     plt.tight_layout()
#     # plt.grid(True) # Disable grid
#     plt.savefig("./A3_result/A3_m.png", dpi=300)
