...
def main():
    ...
    folder = "./Human_Mouse/0_h_ss_score/h_result_1000"
    print(f"Folder path: {folder}")
    a5_df = pd.read_csv("./data/hg19_A5.txt", sep="\t")
    a5_df["index"]    = -1
    a5_df["prob_5ss"] = -1.0
    a5_df["prob_3ss"] = -1.0
    ...
    ax2.legend(
        ...
        bbox_to_anchor=(-0.4, 0.93),  # 往上移一點（y > 1）
        fontsize=40,
        frameon=False
    )
    ...
    plt.tight_layout()
    plt.savefig("./A5_result/A5_h_k.png", dpi=400)
    plt.show()
    print(f"r = {r:.2f}\n(p = {p:.2g})")
...
#     )
#     ax1.set_ylabel("5'SS Score", fontsize=20)
#     ax1.set_xlabel("")
#     ax1.set_xticks([0, 1])
#     ax1.set_xticklabels(
#         ["Minor 5'SS", "Major 5'SS"],
#         fontsize=20
#     )
...
#     # ax1.set_title("Score Distribution", fontsize=13)
#     ax1.set_xlabel("Alternative 5'SS", fontsize=20)
...
#     y = df_extremes["prob_5ss"]
#     colors = df_extremes["type"].map({"major":"blue","minor":"orange"})
#     ax2.scatter(x, y, c=colors, s=40, alpha=0.7)
#     ax2.set_xlabel("PSI", fontsize=20)
#     ax2.set_ylabel("5'SS Score", fontsize=20)
#     # ax2.set_title(f"Pearson r = {r:.2f}, p = {p:.2g}", fontsize=13)
...
#     # 設定 legend
#     # ax2.legend(handles=custom_legend, loc='best', fontsize=20, frameon=False)
#     ax2.legend(
#         handles=custom_legend,
#         loc='upper left',  # 基準點是左上角
#         bbox_to_anchor=(-0.13, 0.9),  # 往上移一點（y > 1）
#         fontsize=20,
#         frameon=False
#     )
...
#     #         plt.Line2D([0],[0], marker='o', color='w', label="Minor", markerfacecolor='orange', markersize=8)
#     #     ],
#     #     title="Event", loc="lower left", fontsize=20
#     # )

#     plt.tight_layout()
#     plt.savefig("./A5_result/A5_h.png", dpi=400)
#     plt.show()
