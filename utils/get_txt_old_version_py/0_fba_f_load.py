...
# 加载 pickle 文件中的字典
with open(f'f_new_{my_seed}.pkl', 'rb') as f:
    data = pickle.load(f)
...
with open(f'f_pred_all_new_{my_seed}.pkl', 'rb') as f:
    data_pred_all = pickle.load(f)

pred_all = data_pred_all['pred_all']
loglik = pred_all[1]

with open(f'f_predictions_new_{my_seed}.pkl', 'rb') as f:
    data = pickle.load(f)
...
# 1938
for i in range(0, 1938):  # 循环遍历指定索引范围
    # 打印当前正在处理的索引，输出到原始标准输出（终端）
    original_stdout.write(f"Running index {i}...\n")
    
    fold_name = f"./{my_seed}_f_result/f_result_{top_k}"
    output_filename = f"./{my_seed}_f_result/f_result_{top_k}/000_fly_g_{i}.txt"  # 注意这里用 i 作为文件名的一部分，避免重复写入同一个文件
    import os
