...
# 加载 pickle 文件中的字典
with open(f'o_new_{my_seed}.pkl', 'rb') as f:
    data = pickle.load(f)
...
with open(f'o_pred_all_new_{my_seed}.pkl', 'rb') as f:
    data_pred_all = pickle.load(f)

pred_all = data_pred_all['pred_all']
loglik = pred_all[1]

with open(f'o_predictions_new_{my_seed}.pkl', 'rb') as f:
    data = pickle.load(f)
...
# 921
for i in range(0, 921):  # 循环遍历指定索引范围
    # 打印当前正在处理的索引，输出到原始标准输出（终端）
    original_stdout.write(f"Running index {i}...\n")
    
    fold_name = f"./{my_seed}_o_result/o_result_{top_k}"
    output_filename = f"./{my_seed}_o_result/o_result_{top_k}/000_moth_g_{i}.txt"  # 注意这里用 i 作为文件名的一部分，避免重复写入同一个文件
    import os
