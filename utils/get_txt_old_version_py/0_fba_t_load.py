...
# 1117
for i in range(0, 1117):  # 循环遍历指定索引范围
    # 打印当前正在处理的索引，输出到原始标准输出（终端）
    original_stdout.write(f"Running index {i}...\n")
    
    fold_name = f"./{my_seed}_t_result/t_result_{top_k}"
    output_filename = f"./{my_seed}_t_result/t_result_{top_k}/000_arabidopsis_g_{i}.txt"  # 注意这里用 i 作为文件名的一部分，避免重复写入同一个文件
    import os
